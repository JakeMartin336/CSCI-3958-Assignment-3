...
app.post('/pokemon/teach/:caughtPokemonId', async (req, res) => {
  ...
  if (!Array.isArray(newMoves) || newMoves.length > 4) {
    return res.status(400).json('Invalid moves input. Provide an array of up to 4 moves.');
  }

  const pokemon = CAUGHT_POKEMON.find(p => p.caughtPokemonId === caughtPokemonId);
  if (!pokemon) {
    return res.status(404).json(`Invalid caughtPokemonId: ${caughtPokemonId}`);
  }
  ...
  if (fetchedMoves.length === 0) {
    return res.status(400).json('No valid moves found from the provided input.');
  }
...
app.put('/pokemon/evolve/:caughtPokemonId', async (req, res) => {
  ...
  if (!response.ok) {
    return res.status(400).json(`Unable to retrieve Pokémon data for id: ${pokemon.id}`);
  }
  ...
  if (!evolutionResponse.ok) {
    return res.status(400).json('Unable to retrieve evolution chain.');
  }
  ...
  if (!currentSpecies || currentSpecies.evolves_to.length === 0) {
    return res.status(400).json(`${pokemon.name} cannot evolve further.`);
  }
  ...
  if (!nextPokemonResponse.ok) {
    return res.status(400).json('Unable to retrieve evolved Pokémon details.');
  }
...
app.post('/pokemon/breed-check', async (req, res) => {
  ...
  if (!Array.isArray(breedPokemon) || breedPokemon.length !== 2) {
    return res.status(400).json('Invalid pokemon input. Provide an array of 2 Pokemon.');
  }
  ...
  if (!pokemon1Response.ok) {
    return res.status(400).json(`Invalid Pokémon: ${pokemon1}`);
  }
  ...
  if (!pokemon2Response.ok) {
    return res.status(400).json(`Invalid Pokémon: ${pokemon2}`);
  }
...
app.get('/item/buy/:id', async (req, res) => {
  ...
  if (!itemResponse.ok) {
    return res.status(400).json(`Invalid item: ${itemInput}`);
  }
  ...
  if (bag.length >= 10) {
    return res.status(500).json('Bag is full. Cannot add more items.');
  }
